...
- source-reviews all 13 CR 609 Effects records and records universal effect
  scope, do-as-much-as-possible, `as though`, tie, and damage-source
  dependencies without promoting partial witnesses;
- verifies that a supplied source-property mismatch does not apply or journal
  an immutable shield, while an unknown replacement predicate fails closed
  instead of accidentally matching;
- source-reviews all 25 CR 608 resolution records and verifies top-of-stack
  resolution after successive passes, untargeted permanent entry under the
  spell controller, and same-object permanent-spell-copy token conversion;
- keeps partial resolution-time target, choice, LKI, APNAP, underlying-card
  movement, final destination, Aura, mutate, and resolution-trigger witnesses
  blocked where their complete dependencies are absent;
- source-reviews all 26 CR 607 linked-ability records and records
  exact-incarnation, chosen-name, undefined-choice, and cost-option witnesses
  without promoting them to a generic linked-ability model;
...
- removes the incorrect planeswalker-only activation restriction, checks base
  ...
  represented;
- source-reviews all 14 CR 605 mana-ability records, excludes targeted and
  loyalty abilities from the activated-mana classifier, and verifies immediate
  stackless activation plus automatic activation during spell payment;
- keeps incomplete possible-output grammar, arbitrary nested payment windows,
  activation reentry, and all generic triggered-mana classification and
  immediate dispatch explicitly blocked;
- source-reviews all 9 CR 604 static-ability records and verifies that a
  represented battlefield effect stops with its source and a represented
  Equipment effect follows the attachment;
...
- source-reviews the sole CR 600 General record as definition-only taxonomy
  and links it to the CR 601-609 dependency contracts;
- source-reviews all 4 CR 513 end-step records, collects represented permanent
  and delayed triggers before priority, normalizes delayed-controller
  sentinels, and verifies no retroactive trigger for a source or delayed
  ability created later in the step;
- verifies multiplayer next-end-step deferral, turn-duration handoff to
  cleanup, and exact command replay while retaining cross-family APNAP trigger
  ordering as an explicit dependency;
- source-reviews all 5 CR 514 cleanup records, verifies exact private discard
  and ordinary no-priority advancement, and implements a serialized
  cleanup-exception frame for state-action/trigger stabilization, exceptional
  priority, and the required additional cleanup step;
- keeps universal simultaneous turn-duration expiration and the complete
  replacement, state-action, trigger, APNAP, multiplayer, hidden-information,
  and replay interaction matrix explicitly blocked;
- source-reviews all 20 CR 602 activation records, applies summoning sickness
  ...
  changes;
- verifies activated-only classification and sorcery/instant timing while
  leaving full cost/instruction grammar, transactional rollback, opponent
  choices, cost-altering effects, and acquired-ability provenance explicitly
  blocked;
...
- Inventory-only cases: 2,984
- Reviewed blocked cases: 222
- Reviewed definition-only cases: 50
- Executable semantic passes: 44
- Indexed sections: 156
- Glossary entries: 733
- Discovered mechanics: 425
- Partial/untrusted mechanic contracts: 28
- Unclassified mechanics: 397
...
- Unit/integration tests: 3,785 passed
- Noninventory unit/integration tests: 485 passed
...
- Focused CR 600 taxonomy tests: 1 passed
- Focused CR 513 end-step tests: 5 passed
- Focused CR 514 cleanup-step tests: 7 passed
...
- Wheel SHA-256:
  `eca785e364962fc1f76cdd0651413441e56db2cdf70d038369b9b21b144f27ef`
